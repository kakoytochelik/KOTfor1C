...
| `kotTestToolkit.editor.autoFillNestedScenariosOnSave` | Автозаполнение блока `ВложенныеСценарии` (только при изменении вызовов или неполной секции) |
| `kotTestToolkit.editor.autoFillScenarioParametersOnSave` | Автозаполнение блока `ПараметрыСценария` (только при изменении параметров или неполной секции) |
| `kotTestToolkit.editor.scenarioParameterExclusions` | Исключения для значений в `[]`, против ложного распознавания параметров |
...
- Для постоянной работы используется локальная диагностика (активный файл + связанные родительские сценарии).
- Глобальное сканирование (`scanWorkspaceDiagnostics`) запускайте вручную, когда действительно нужно.
- При глобальном сканировании подсказки `Maybe you meant` не рассчитываются (это сделано для снижения нагрузки).
...
## 10) Актуальные примечания по редактору

- Допустимы параметры в квадратных скобках без кавычек: `Step [Parameter]`.
- Для вызовов вложенных сценариев в hover показываются:
  - описание из `KOTМетаданные.Описание`,
  - количество файлов в папке `files`,
  - количество параметров и вложенных вызовов.
- Текст внутри `KOTМетаданные -> Описание` подсвечивается в редакторе.
- Диагностика проверяет дубли `ДанныеСценария.Код` и показывает предупреждения по всем найденным конфликтам.
