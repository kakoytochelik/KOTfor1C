...
- Установка runtime-расширения Form Explorer ускорена при повторных запусках: KOT переиспользует уже подготовленные артефакты и не пересобирает расширение без необходимости.
- При `Start infobase`, `Open with Form Explorer` и ручной установке runtime KOT теперь предлагает выбрать режим установки прямо в момент запуска:
  ...
  - `target` сначала выгружает конфигурацию самой выбранной базы, затем генерирует runtime уже по этой выгрузке и устанавливает его обратно в ту же базу; для файловых баз KOT сначала пытается сделать выгрузку через `ibcmd`, а при недоступности или ошибке автоматически откатывается на Designer; это самый медленный, но и самый точный режим, когда база больше не соответствует текущей ветке.


## Vanessa Automation и служебная startup-ИБ

- При первом запуске Vanessa Automation более не запрашивается изменение `conf.cfg`:
  - на Windows startup-ИБ теперь поднимается из шаблона `tools/startup-infobase/KOTStartupTemplate.dt`, который идет внутри расширения;
  - подход не требует прав администратора Windows и устраняет предупреждение безопасности первого запуска внешней обработки в служебной базе;
  - настройка `kotTestToolkit.runVanessa.checkUnsafeActionProtection` убрана из settings, потому что расширение больше не рассчитывает на ручное отключение защиты через глобальный `conf.cfg`.

## Редактор и навигация

- В контекстное меню редактора YAML-сценария, в подменю `Work With Files`, добавлена команда открытия папки `files` текущего открытого сценария в системном проводнике.
