...
- При `Start infobase`, `Open with Form Explorer` и ручной установке runtime KOT теперь предлагает выбрать режим установки прямо в момент запуска:
  ...
  - `cfe` медленнее, но безопаснее по совместимости и лучше подходит для баз, которые могут заметно отличаться от конфигурации в репозитории, этот режим собирает и устанавливает runtime через пакет `.cfe`.
...
- При первом запуске Vanessa Automation более не запрашивается изменение conf.cfg:
  ...
  - подход не требует прав администратора Windows и устраняет security warning первого запуска внешней обработки в служебной базе.
  - настройка `kotTestToolkit.runVanessa.checkUnsafeActionProtection` убрана из settings, так как
...
- `СборкаТекстовСценариев` и встроенный запуск Vanessa переведены на общую lightweight startup-ИБ.
