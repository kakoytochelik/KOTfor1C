...
### KOT Form Explorer (beta)

- Добавлен новый блок `KOT Form Explorer`:
  - webview-панель в VS Code для исследования текущей управляемой формы 1С;
  - one-click генерация runtime-расширения `.cfe` по файловой выгрузке `cf`;
  - static enrichment snapshot-а по `Form.xml` и metadata конфигурации;
  - beta-маркировка в интерфейсе и документации.
- Добавлены команды:
  - `KOT - Open 1C Form Explorer`
  - `KOT - Generate Form Explorer extension project`
- Добавлена кнопка открытия `KOT Form Explorer` в меню `...` Test Manager, в отдельной секции под `Open Vanessa`.
- Реализован lightweight runtime `.cfe` без изменения основной конфигурации:
  - отдельная подсистема `KOT Form Explorer`;
  - startup-инициализация через `ManagedApplicationModule`;
  - форма настроек адаптера;
  - preset-hotkeys для ручного snapshot-а;
  - `manual` / `auto` режимы обновления.
- Добавлена двусторонняя синхронизация режима между VS Code и 1С:
  - `adapter-mode.txt` для фактического состояния;
  - `adapter-mode-request.txt` для запроса переключения из VS Code.
- Добавлен встроенный Windows builder `.cfe`:
  - генерация source tree расширения;
  - кэшируемая builder-ИБ;
  - повторное использование загруженной основной конфигурации;
  - выгрузка готового `.cfe` через `DumpCfg -Extension`.
- Добавлено автоопределение установленной платформы 1С и автоподстановка пути к `1cv8c.exe` в настройки.
- Подготовка служебных ИБ переведена в фоновый прогрев на старте расширения и при изменении пути к клиенту 1С.
- Добавлен optional override внешней сборки через `extensionBuildCommandTemplate`, а также helper-скрипты для `PowerShell` и `OneScript`.

### Vanessa Automation: трекинг внешних прогонов по run-логу

Помимо трекинга тестов, запущенных непосредственно из расширения, теперь есть возможность отслеживать любой запущенный тест (в том числе полезно для трекинга процесса прохождения тестов на сервере).
...
- Определение ошибки во внешнем трекинге выполняется по строке `Failed:` в run-логе.
- Интервал опроса run-логов унифицирован и берется из `kotTestToolkit.runVanessa.liveLogRefreshSeconds` для всех режимов live-трекинга.
...
- `KOT Form Explorer` оставлен на отдельной builder-ИБ с загруженной конфигурацией.
- Добавлена настройка `kotTestToolkit.formExplorer.showOutputPanel` для управления автооткрытием Output при сборке `.cfe` и прогреве builder-ИБ.
