...
- **Удалено**:
    ...
    - **Настройка BuildScenarioBDD Parameters:** Удалена, так как все параметры теперь управляются через Менеджер параметров Сборки Сценариев.
...
- **Новые возможности**:
    ...
    - **Настраиваемые пути проекта:**
        ...
        - Опциональные EPF файлы (`RepairTestFile.epf`) - обработка пропускается, если путь не задан.
