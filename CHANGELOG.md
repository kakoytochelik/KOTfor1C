...
- Добавлен каталог платформ 1С и отдельный менеджер платформ. Его можно открыть через настройки или из меню действий Test Manager.
...
- Добавлена настройка, позволяющая отключить выбор платформы при запуске и использовать платформу по умолчанию.
...
- Добавлен менеджер эталонных баз для редактирования `bases.yaml` как источника `ModelDBSettings` в параметрах СППР.
...
- При создании главного сценария теперь можно выбрать эталонную базу и профиль пользователя. В `test.yaml` отдельно сохраняются имя эталонной базы, идентификатор базы и профиль пользователя.
...
- Для установки и запуска Form Explorer добавлены режимы `direct`, `cfe` и `target`:
  - `direct` выбран по умолчанию и подходит для самого быстрого старта, когда целевая база соответствует текущей ветке или хотя бы не сильно отличается от нее, этот режим загружает сгенерированное расширение напрямую в выбранную базу;
  - `cfe` это промежуточный режим: он по-прежнему использует конфигурацию из ветки, но устанавливает runtime через builder-ИБ и пакет `.cfe`, поэтому работает медленнее `direct`, зато обычно предсказуемее;
  - `target` сначала выгружает конфигурацию самой выбранной базы, затем генерирует runtime уже по этой выгрузке и устанавливает его обратно в ту же базу; это самый медленный, но и самый точный режим, когда база больше не соответствует текущей ветке.
...
- Первый запуск Vanessa Automation больше не требует ручной правки `conf.cfg`, что могло вызвать проблемы при отсутствии системных прав администратора.
...
- При выборе восстановления доступны наряду с `No DT restore` и `Restore from another DT`, еще и `Restore from etalon DT` (если найден `DT` из `bases.yaml`, который указан в `ModelDBSettings`), что ближе к процессу работы в СППР.

### Менеджер параметров и JSON-артефакты

- В менеджере параметров появились профили наборов настроек с автосохранением.
...
- Старая модель runtime-путей заменена на `kotTestToolkit.runtime.directory`, по умолчанию использующий `.vscode/kot-runtime`.
...
- Для YAML-файлов сценариев (`ТипФайла: "Сценарий"`) добавлено автоматическое переключение language mode в `Feature/Gherkin`, чтобы редактор использовал привычную подсветку синтаксиса без ручного `Configure File Association`. Это поведение переключается в настройках.
- В контекстное меню YAML-сценария добавлена команда открытия папки `files`.
- В шапке главного сценария добавлено быстрое действие открытия связанного `test.yaml`.
- Hover-подсказки расширены на `test.yaml`, а также блоки параметры теста, параметры сценария и вложенные сценарии.
- Для `test.yaml` добавлены быстрые редакторные действия для смены эталонной базы, профиля пользователя и связанного сценария.
...
- Добавлен исследователь форм 1С `KOT Form Explorer` ([#26](https://github.com/kakoytochelik/KOTfor1C/issues/26)):
  - панель в VS Code для исследования текущей управляемой формы 1С;
  - автоматический или ручной режим обновления информации о форме;
  - доступна информация о текущей форме, ее элементах и значениях, таблицах Gherkin;
  - поиск элементов по тексту или по клику в интерфейсе 1С;
  - предложения шагов Vanessa Automation для текущего элемента.
- При запуске базы из интерфейса `KOT Form Explorer` автоматически устанавливается `.cfe`-адаптер для связи с расширением.
- В snapshot selector `KOT Form Explorer` отображается имя ИБ из лаунчера 1С, если путь базы удалось сопоставить с `ibases.v8i`.
- Добавлена кнопка открытия `KOT Form Explorer` в меню `...` Test Manager.

_Функционал был проверен на конфигурации 1C:Drive и может содержать ошибки при работе с другими конфигурациями. Просьба оставлять issue при обнаружении проблем._

### KOT Infobase Manager

- Добавлен отдельный `KOT Infobase Manager` ([#25](https://github.com/kakoytochelik/KOTfor1C/issues/25)):
  - единый список ИБ из лаунчера 1С, runtime-артефактов, snapshot-ов и вручную добавленных путей;
  - для базы показываются роли, источник, последний запуск, последний snapshot, последний run-log и текущее состояние;
- В менеджере баз доступны операции:
  - создать, копировать, пересоздать, восстановить из `DT`, выгрузить в `DT`, сохранить конфигурацию в `.cf`, обновить конфигурацию из папки или `.cf`;
  - добавить/убрать из лаунчера 1С, открыть базу в 1С или Designer, открыть папку и связанные логи;
  - переименовать базу, перенести или переназначить каталог, полностью удалить файлы и запись из лаунчера 1С.
- Для каждой базы доступна отдельная настройка ключей запуска 1С.
- Список баз поддерживает режимы сортировки `Last opened / Alphabetical / Manual order`.
- Для `KOT Form Explorer` и подготовки запуска тестов Vanessa используется общий механизм:
  - picker работает с единым списком обнаруженных баз и умеет сразу открыть `KOT Infobase Manager`;
  - при установке/запуске `Form Explorer` и при запуске тестов Vanessa в менеджер сохраняются роли и метаданные последнего использования базы.

### Vanessa Automation: внешний трекинг и подготовка целевой ИБ

Помимо ранее реализованного трекинга тестов, запущенных непосредственно из расширения, теперь есть возможность отслеживать любой запущенный тест (в том числе полезно для трекинга процесса прохождения тестов на сервере).

- Добавлено автоопределение активных прогонов Vanessa по свежему появлению или обновлению run-логов в `kot-runtime/vanessa`:
  - старые лежащие в папке логи больше не считаются актуальным прогоном;
  - если старый лог перезаписывается новым прогоном, статус автоматически стартует заново;
  - единичное обновление run-лога на старте можно отфильтровать, чтобы не подхватывать случайные ручные шаги;
  - автоопределенный трекинг автоматически снимается по таймауту неактивности;
  - при сопоставлении лога со сценарием из Test Manager статус подхватывается без ручного запуска трекинга.
- Добавлены команды контекстного меню редактора:
  - `KOT - Track scenario run by log for opened feature` — выбрать лог и начать трекинг для открытого `feature`.
  - `KOT - Switch tracked run for opened feature` — быстро переключить активный отслеживаемый прогон.
  - `KOT - Stop tracked runs for opened feature` — остановить внешнее отслеживание для открытого `feature`.
  - `KOT - Track run by opened log file` — запустить трекинг прямо из открытого `.log` (по заголовку `Feature: ...`) с автопереходом к соответствующему `feature`-файлу.
- Для одной `feature` поддержано несколько параллельно отслеживаемых прогонов с удобным переключением по пути к логу.
- Для запуска тестов из Test Manager добавлена подготовка целевой ИБ:
  - можно выбрать существующую файловую ИБ из лаунчера 1С или создать новую;
  - при создании новой ИБ путь выбирается через системный выбор папки, затем запрашивается имя базы для лаунчера 1С и после успешного создания база автоматически регистрируется в `ibases.v8i`;
  - для существующей и новой ИБ доступны диалоги восстановления из `DT` и обновления конфигурации;
  - существующую файловую ИБ можно пересоздать с нуля в той же папке перед подготовкой;
  - конфигурацию можно обновить либо из `kotTestToolkit.formExplorer.configurationSourceDirectory`, либо из выбранного `.cf`;
  - для новой базы запуск блокируется, если пользователь не выбрал ни восстановление из `DT`, ни обновление конфигурации;
  - подготовка (`create`, `restore DT`, `update configuration`) сопровождается пошаговой progress-индикацией.
...
- Убраны лишние интерфейсные элементы.

### Другие улучшения

- Наличие пользовательской служебной базы для запуска Сборки сценариев или Vanessa Automation более не требуется, расширение само создает базу в фоновом режиме.
- Добавлено автоопределение установленной платформы 1С и автоподстановка пути к `1cv8c.exe` в настройки.
...
- `СборкаТекстовСценариев` и встроенный запуск Vanessa переведены на общую легковесную startup-ИБ.
...
- Улучшена работа `Test Manager` при подключении через VS Code `Remote Tunnel`:
  - webview панели теперь рендерится сразу, без ожидания полного первичного прогрева кеша сценариев;
  - список главных сценариев отдается в интерфейс раньше, а синхронизация run artifacts и `Favorites` догружается асинхронно после первого рендера.
- Ускорен серверный поиск сценариев и построение кеша в remote-среде:
  - обход файлов в `yamlSourceDirectory` переведен на прямое чтение через серверный `fs` с fallback на `workspace.fs`;
  - поиск `scen.yaml` и навигационный fallback больше не зависят от `workspace.findFiles`, что уменьшает задержки на удаленном extension host.
- Доработана навигация между сценариями при медленном или еще не завершенном cold start:
  - команды открытия сценариев и переходы из `hover`/`feature` используют прямой server-side поиск файла, даже если полный кеш еще не готов;
  - fallback-поиск теперь использует фактический `kotTestToolkit.paths.yamlSourceDirectory`, а не жестко заданный `tests/**/*.yaml`.
- Улучшена непрерывность live-подсветки шагов для встроенных запусков тестов Vanessa Automation.
- Доработано IntelliSense переменных по символу `$`: обычные переменные подставляются как `$VariableName$`, пользовательские `GlobalVars` из Менеджера параметров как `$$GlobalVar$$`, при ручном вводе `$$` в подсказках показываются только глобальные переменные, а в выдаче IntelliSense и hover по переменной теперь отображается ее значение. ([#29](https://github.com/kakoytochelik/KOTfor1C/issues/29))
- Расширено и уточнено определение шагов, создающих переменные: IntelliSense и hover теперь учитывают не только базовые `save/запоминаю`, но и дополнительные шаблоны сохранения, копирования и `find or create`/`нахожу или создаю` в локальный и глобальный контекст, при этом шаги с `key/ключ` больше не считаются обычными переменными.
- Расширено распознавание управляющих блоков в диагностике сценариев: теперь учитываются `If/ElseIf/Else/EndIf`, `Do/EndDo`, `Try/Except/EndTry`, русские аналоги, а также циклические формы шагов вроде `And ... I do`, `And ... I execute`, `И ... я выполняю`, `И пока ... тогда`.
- Для открывающих управляющих конструкций в IntelliSense добавлена автоподстановка закрывающей строки: при выборе `If`/`Do`/`Try` и русских аналогов вставляется блок с пустой строкой внутри и соответствующим `EndIf`/`EndDo`/`EndTry` или `КонецЕсли`/`КонецЦикла`/`КонецПопытки`.
- Доработаны Gherkin-таблицы: при вставке многострочных шагов из IntelliSense строки таблицы теперь автоматически получают корректный относительный отступ от строки шага, а выравнивание таблиц на сохранении дополнительно нормализует и их собственный отступ.
- Доработано создание новых YAML-сценариев:
  - служебные поля `ДанныеСценария` и `ДанныеТеста` теперь подставляются из `kotTestToolkit.newScenarioDefaults`, а при создании можно выбрать: взять defaults или вручную пройти по оставшимся полям шапки;
  - для `ФункцияСистемы` добавлен отдельный каталог и UI-менеджер, а в YAML всегда согласованно записываются `ФункцияСистемы` и `UIDФункцияСистемы`; для новой записи UID генерируется автоматически;
  - при создании главного сценария выбор группы в Test Manager теперь показывает существующие группы и позволяет сразу ввести новую;
  - при создании главного сценария отдельно запрашивается `ModelDBid` для `test.yaml`, который записывается в `ЭталоннаяБазаИмя` и `ИдентификаторБазы`;
  - в шапке YAML для `ФункцияСистемы` и `UIDФункцияСистемы` добавлен быстрый `✎`-переключатель: при выборе новой функции системы оба поля обновляются синхронно;
  - в hover по YAML добавлены описания ключей шапки сценария, `test.yaml` и `KOTМетаданные` ([#21](https://github.com/kakoytochelik/KOTfor1C/issues/21)).
- В контекстное меню теста в `Test Manager` добавлены действия:
  - открыть файл настроек теста `test/<Имя>.yaml`;
  - удалить главный сценарий целиком с подтверждением, включая папку сценария, `test`, `files` и очистку связанного состояния расширения.

# 2.3.1

## Новые возможности

### Vanessa Automation: live-подсветка выполнения шагов в feature-файле

- Добавлена live-подсветка шагов в `feature` во время прогона:
  - текущий шаг;
  - пройденные шаги (зеленым);
  - упавший шаг (красным);
  - после завершения теста подсветка сохраняется до изменения или до пересборки `feature`-файла.

### Навигация по ошибкам и вложенным сценариям

- Для упавшего шага добавлен расширенный hover с полным блоком ошибки.
- В hover добавлены действия `Open run log` и `Open failed nested scenario`.
- Добавлены быстрые переходы:
  - `Open failed step` (к строке ошибки в `feature`);
  - `Open failed nested scenario` (к вложенному сценарию по контексту отступов).
- Для `feature` добавлена команда контекстного меню: открыть вложенный сценарий по позиции курсора.

### Test Manager

- Добавлен прогресс выполнения по строкам `feature`:
  - проценты в строке сценария;
  - счетчик шагов `x/y` при наведении.
- Для `failed` сценария добавлена отдельная кнопка перехода к упавшему шагу в `feature`.
- Изменено поведение клика по строке сценария, когда сценарий запущен — открывается `feature` в live-режиме на текущем шаге;

### Базовый IntelliSense и hover в feature-файлах

- В `feature`-файлах включены:
  - IntelliSense по шагам Vanessa (включая семантический поиск по префиксу `!`);
  - IntelliSense по переменным;
  - hover-подсказки по шагам и вызовам сценариев.

## Исправления и доработки

- Когда запущен хотя бы один тест, блокируются `Build tests` и чекбоксы выбора (чтобы не затерлись файлы во время прохождения теста);
- Во время старта прогона Output панель больше не открывается автоматически (меньше информационого шума).
- Для кнопки запуска упавшего теста добавлено визуальное состояние retry (иконка `play` при наведении).
- Добавлен опциональный legacy-режим сборки: при `Build tests` временно выносятся `test/*.yaml` выключенных сценариев в системную временную папку и автоматически возвращаются после завершения сборки.

# 2.2.1

### UI: Test Manager

- Кнопки редких действий в шапке `Test Manager` реорганизованы в единый блок `More actions` (`...`).
- Добавлены команды `Refresh steps library` (`Обновить библиотеку шагов`) и `Repair scenario files` (`Исправление файлов сценариев`) в меню панели Менеджера тестов.
- Для лучшей ясности, текст статуса выбора тестов изменен с `Selected: X/Y` на `Will be built: X/Y`.

### Пакетное исправление и проверка сценариев

- Добавлены команды для исправления форматирования сценариев:
  - `KOT - Repair and validate changed scenarios (safe batch)` / `KOT - Исправить и проверить измененные сценарии (безопасный пакетный режим)` - используются сохраненные измененные сценарии и измененные в Git `.yaml` файлы;
  - `KOT - Repair and validate all scenarios in YAML source directory (high load)` / `KOT - Исправить и проверить все сценарии в yamlSourceDirectory (высокая нагрузка)` - обрабатываются все сценарии в `yamlSourceDirectory`;
  - Данные команды доступны в меню `More actions` (`...`) в панели Менеджера тестов.

- Исправлена критическая ошибка, случайным образом приводящая к потере текстов сценариев при использовании функции VSCode `Replace in Files` с включенным Auto Save:
...
- Добавлены интеллектуальные семантические подсказки шагов в IntelliSense по префиксу `!`:
  - можно вводить формулировку желаемого шага своими словами, например: `!условие, что открылось окно`, `!проверка фоновых заданий`, `!сохранить выражение в переменную`, и плагин предложит соответствующие шаги из библиотеки шагов Vanessa Automation;
  - для подсказок шагов установлен приоритет языка по тегу `#language:` в текущем сценарии.

### Управление кодом и именем сценария:

- Добавлена команда `KOT - Изменить имя главного сценария`:
  - за одно действие меняет `ДанныеСценария.Имя` и `ДанныеСценария.Код`
  - автоматически переименовывает папку главного сценария.
  - обновляет список в Менеджере тестов.
- Добавлена команда `KOT - Изменить имя вложенного сценария`:
  - за одно действие меняет `ДанныеСценария.Имя` в `scen.yaml`, а также все вызовы данного сценария во всех связанных тестах;
...
- Данные команды доступны как через контекстное меню в `KOT - Редактирование`, так и из самого редактора по иконке карандаша напротив соответствующих полей.

### Навигация по вызовам вложенных сценариев:

- Открыть вложенные сценарии теперь можно не только через контекстное меню: в hover по вызову вложенного сценария добавлена ссылка `Open scenario` / `Открыть сценарий`.

## Исправления и доработки

- Улучшено автосворачивание секций `ВложенныеСценарии` и `ПараметрыСценария` при открытии файла:
  - сворачивание больше не выполняется через временное перемещение курсора;
  - устранены визуальные “прыжки” выделения/позиции при открытии сценария.
...
- **Интеграция сборки тестов в TypeScript**:
    ...
    - [BETA] Сборка тестов теперь доступна через встроенный launcher расширения.
