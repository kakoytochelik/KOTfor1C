...
- Реализован устанавливаемый автоматически `.cfe`-адаптер для связи с VSCode-расширением:
  - Установка происходит при запуске базы из интерфейса `KOT Form Explorer`.
...
### Vanessa Automation: трекинг внешних прогонов по run-логу
...
- Весь интерфейс плагина был обновлен;
...
- При встроенном запуске теста из Test Manager переработан выбор тестируемой базы:
  - вместо простого ручного ввода пути предлагается выбор существующей ИБ из лаунчера 1С или создание новой файловой ИБ;
  ...
  - существующую файловую ИБ можно пересоздать с нуля в той же папке перед подготовкой, чтобы получить чистую базу без ручного создания новой директории;
...
- Для подготовки целевой ИБ перед built-in запуском Vanessa добавлена пошаговая progress-индикация (`create`, `restore DT`, `update configuration`).
- В snapshot selector `KOT Form Explorer` подпись базы теперь берется из имени ИБ в лаунчере 1С, если для snapshot удалось сопоставить путь базы с `ibases.v8i`.
