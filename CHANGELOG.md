...
- Добавлено автодополнение переменных по символу `$`: предлагаются переменные, ранее сохраненные шагами `I save ... in "..." variable` / `Я запоминаю значение выражения ... в переменную "..."`, с подстановкой в формате `$VariableName$`.
