...
- Семантический режим по префиксу `!`:
  ...
  - Приоритет языка подсказок шагов определяется тегом `#language:` в текущем сценарии (второй язык показывается ниже).
...
- Если обнаружены дубликаты кода сценария (поле `Код` в шапке), через контекстное меню можно изменить код открытого сценария: `KOT - Редактирование` → `KOT - Изменить код вложенного сценария`.
