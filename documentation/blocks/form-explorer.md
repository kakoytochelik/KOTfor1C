...
2. Выбрать базу из общего списка менеджера баз (`launcher`, `runtime`, `snapshot`, ручные пути) или указать путь вручную;
...
При необходимости базу можно заранее подготовить через команду `KOT - Open Infobase Manager`: создать новую, пересоздать существующую, выгрузить/восстановить `DT`, обновить конфигурацию и затем использовать ту же базу в `Start infobase`.
