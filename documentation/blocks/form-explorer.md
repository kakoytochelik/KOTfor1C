...
- выбирать snapshot из списка, вручную подменять snapshot-файл и возвращаться к настроенному пути;
...
4. В выборе режима запуска доступны варианты:
   ...
   `Build/install via .cfe ...` для более безопасного по совместимости сценария;
5. Для `direct` режима KOT строго рекомендует использовать базы, которые соответствуют текущей ветке или хотя бы не сильно отличаются от `configurationSourceDirectory`;
6. Для `cfe` режима KOT собирает или переиспользует кэшированный `.cfe` через отдельную builder-ИБ и потом ставит его в выбранную базу.

_В обоих режимах источник сборки адаптера берется из `configurationSourceDirectory`. Выбранная база используется как место установки и запуска, а не как источник метаданных для генерации._

7. Дождаться открытия базы.
...
Если путь базы удалось сопоставить с записью в лаунчере 1С, в selector показывается имя базы из launcher, а не только имя папки файловой ИБ.
...
При `direct` режиме, который выбирается интерактивно при установке или запуске, шаги с builder-ИБ и `.cfe` пропускаются: generated extension загружается прямо в выбранную ИБ через `/LoadConfigFromFiles ... -Extension`. Это быстрее, но такой режим строго рекомендуется только когда целевая база соответствует `configurationSourceDirectory` или хотя бы не сильно отличается от него.

При `cfe` режиме KOT либо переиспользует кэшированный `.cfe`, либо пересобирает его в builder-ИБ. Этот режим медленнее, но безопаснее, если база могла отстать от ветки, расходиться по метаданным или просто вызывает сомнения по совместимости.
