...
- исходники конфигурации в файловом формате - для опциональной генерации и сохранения `.cfe` в папку.
...
4. (_Если процесс уже был пройден раньше_) Будет запрос - запустить базу как есть, или переустановить расширение. Рекомендуется переустановить, если были внесены изменения в конфигурацию базы.;

_В этот момент происходит сборка и установка расширения-адаптера, связанного с конфигурацией выбранной базы._
...
3. Сгенерированное расширение загружается через `-Extension`;
4. `.cfe` выгружается через `DumpCfg -Extension`.

Это сильно ускоряет повторные сборки.

При запуске базы через `Start infobase` сборка `.cfe` происходит автоматически на основе конфигурации выбранной базы, и процесс идет гораздо быстрее.
