...
3. Глобальный scan по команде: `KOT - Scan workspace diagnostics` (`KOT - Выполнить сканирование диагностики по проекту`). Перед запуском показывается явное предупреждение о высокой нагрузке; сканирование идет по сценариям из `yamlSourceDirectory`.
