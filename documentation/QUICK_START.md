# Quick Start

Цель этого гайда: за 10-15 минут дойти до рабочего цикла
`Создание и редактирование YAML -> сборка -> запуск в Vanessa`.

## 1) Установка и открытие проекта

1. Установите расширение `KOT for 1C` из Marketplace.
2. Откройте корень проекта в VS Code.
3. Если проект уже содержит сценарии, убедитесь, что они имеют формат теста (`ТипФайла: "Сценарий"`).

## 2) Минимально необходимые настройки

Откройте настройки расширения в VS Code и задайте минимум:

| Настройка | Для чего нужна |
|---|---|
| `kotTestToolkit.platforms.catalog` | Каталог платформ 1С для запусков; обычно KOT заполняет его автоматически найденными установленными версиями |
| `kotTestToolkit.paths.buildScenarioBddEpf` | Путь к обработке `СборкаТекстовСценариев.epf` |
| `kotTestToolkit.runVanessa.vanessaEpfPath` | Путь к `vanessa-automation.epf` |

Рекомендуется сразу указать также:

- `kotTestToolkit.runtime.directory`, если не подходит default `.vscode/kot-runtime`
- `kotTestToolkit.startupParams.parameters`

Если KOT удалось автоматически найти установленную платформу 1С и заполнить каталог платформ, расширение само подготовит в фоне служебную ИБ для запуска Vanessa и `СборкаТекстовСценариев`.

### 2.1) Менеджер параметров (обязательно для сборки и запуска)

Откройте Менеджер параметров через `Test Manager` -> `More actions` -> `Open Build Scenario Parameters Manager` или командой `KOT - Open Build Scenario Parameters Manager` (`KOT - Открыть Менеджер параметров Сборки Сценариев`).

Что важно знать:

1. Вкладка `СППР` формирует `.json` файл, который использует `СборкаТекстовСценариев`.
2. Вкладка `Additional Vanessa Automation parameters` (`Дополнительные параметры`) нужна для VAParams-ключей, которые не покрывает СППР. Сюда можно загрузить ваши собственные настройки Vanessa Automation.
3. Вкладка `GlobalVars` добавляет пользовательские глобальные переменные в параметры запуска.
4. Для строк во вкладках `Additional` и `GlobalVars` можно включать `Override existing value`, если нужно точечно перезаписать одноименный ключ.
5. В репозитории есть пример файла настроек СППР для ознакомления/адаптации под свои нужды: [yaml_parameters.json](../res/yaml_parameters.json)
6. Справка по параметрам обработки `СборкаТекстовСценариев` (СППР): [Официальный портал ИТС](https://its.1c.ru/db/sppr2doc#content:124:hdoc)
7. Вкладка `СППР` фиксирует сверху строки `ScenarioFolder`, `FeatureFolder`, `VanessaFolder`, `ModelDBSettings` и `AuthCompile`, остальные известные параметры можно добавлять из выпадающего списка с описанием, а для модифицированной обработки разрешены custom-строки.
8. Менеджер параметров сохраняет изменения автоматически, синхронизирует внутренний корень сканирования из `ScenarioFolder` и пересканирует список тестов.
9. В шапке менеджера можно переключать профили параметров; активный профиль тоже сохраняется автоматически.
10. Тот же активный профиль можно быстро переключить из выпадающего меню `Build tests` в Test Manager.

Минимум на старте:

- убедитесь, что в СППР-вкладке заполнены минимум `ScenarioFolder` или `TestFolder`, а также `FeatureFolder`;
- заполните `VanessaFolder`, если он не подставился автоматически из `Run Vanessa: Vanessa Epf Path`;
- заполните launch-базу (`LaunchDBFolder` / `TestClientDBPath` / `InfobasePath` / `TestClientDB`), иначе generated JSON для Vanessa не получит рабочий путь запуска;
- если импортировали JSON с `TestFolder`, в UI менеджера он будет показан как `ScenarioFolder`;
- если хотите автоматическое разрешение логинов/паролей `TestClient` по профилю, заполните `ModelDBSettings`; при `AuthCompile=true` это уже обязательно;
- дождитесь, пока менеджер параметров сохранит изменения автоматически (`Updated at` в шапке), и после этого запускайте первую сборку.

## 3) Первый запуск панели

1. Откройте Activity Bar -> `KOT for 1C`.
2. Проверьте, что загрузились группы и сценарии.
3. При необходимости включите поиск по главным сценариям кнопкой с лупой в шапке Test Manager (`Менеджер тестов`).
4. Если нужно проверить или поправить каталог платформ 1С, откройте `...` -> `Manage platforms`.
5. Если это новый проект без готовых YAML-сценариев, пустой список на этом этапе — это нормально.
6. Если сценарии в проекте уже есть, но список пустой, проверьте `ScenarioFolder` в Менеджере параметров и структуру `scen.yaml`.

## 4) Создание первых сценариев (если проект новый)

1. Создайте главный и вложенный сценарий через иконку "+" в шапке Менеджера тестов, или через команды:
   - `KOT - Create main scenario` (`KOT - Создать главный сценарий`) — из сценариев такого типа формируются итоговые `.feature` файлы при сборке.
   - `KOT - Create nested scenario` (`KOT - Создать вложенный сценарий`) — сценарии такого типа можно вызывать из других вложенных сценариев и из главных.
2. Что запросит мастер создания:
   - Для вложенного сценария:
     - имя сценария;
     - числовой код;
     - родительская папка, куда будет создана папка сценария.
   - Для главного сценария:
     - имя сценария (и имя папки);
     - имя группы в Test Manager (`Менеджер тестов`);
     - порядок в группе (опционально);
     - состояние чекбокса по умолчанию (включен/выключен);
     - родительская папка, где будет создан сценарий.
3. При вводе имени и кода действует проверка уникальности (дубликаты не допускаются).
4. После создания обновите панель (если вдруг список не обновился автоматически): `Reload tests from disk` (`Перезагрузить список тестов с диска`).
5. Добавьте вызовы вложенных сценариев в главные сценарии, начав вводить название этого сценария. IntelliSense предложит автозавершение строки.

## 5) Параметры: быстрый минимум

Что нужно для старта:

1. Объявляйте параметр в тексте как `[ParamName]`.
2. Передавайте его в вызов вложенного сценария строкой `ParamName = "Value"` (или `ParamName = [OtherParam]` для сквозной передачи).
3. Если параметр не передан явно, будет взят дефолт из `ПараметрыСценария -> Значение`.

Мини-пример:

```yaml
   ТекстСценария: |
      #language: en

      And Create sales invoice
         DocumentNumber = "000000123"
         Counterparty   = [Counterparty]
```

Важно:

- если квадратные скобки нужны как обычный текст, экранируйте их: `\[` и `\]`;
- quick fix для `Missing parameters` может автоматически добавить отсутствующие параметры в вызов.

Подробно (включая многоуровневую передачу, дефолты, правила и подсказки переменных):  
[`blocks/scenario-parameters.md`](./blocks/scenario-parameters.md)

## 6) Первый build

1. В Test Manager (`Менеджер тестов`) отметьте нужные главные сценарии.
2. Нажмите `Build tests` (`Собрать тесты`).
3. После завершения можете открыть feature-файлы или папку результатов из уведомления.

Результат:
- для выбранных тестов будут сформированы `.feature`;
- рядом будут подготовлены `.json` для запуска Vanessa.

## 7) Первый запуск теста в Vanessa

Есть два способа:

1. После завершения сборки, или если в директории уже содержится `.feature` нужного теста, будет иконка запуска рядом с конкретным сценарием (быстрый запуск).
2. В верхней части Test Manager (`Менеджер тестов`) есть кнопка `Open Vanessa` с выбором сценария и режима:
   - автоматический прогон;
   - `manual debug` — открыть Vanessa для интерактивной отладки без автозакрытия.
3. Для встроенного запуска расширение предложит выбрать тестируемую ИБ:
   - существующая файловая или серверная ИБ из общего списка KOT или вручную;
   - либо создание новой файловой ИБ.
4. Для выбранной ИБ можно дополнительно:
   - восстановить базу из `DT`;
   - обновить конфигурацию из `kotTestToolkit.formExplorer.configurationSourceDirectory` или из выбранного `.cf`.
5. Если выбрана новая ИБ, но не указано ни восстановление из `DT`, ни обновление конфигурации, запуск будет остановлен как бессмысленный для пустой базы.

## 8) Менеджер баз: быстрый старт

`KOT Infobase Manager` полезен, если нужно один раз нормально подготовить базу и потом переиспользовать ее и для Vanessa, и для `KOT Form Explorer`.

Минимальный сценарий:

1. Откройте `KOT - Open Infobase Manager` (`KOT - Открыть Менеджер баз`) или кнопку с иконкой базы в шапке `Test Manager`.
2. Выберите существующую базу из списка или создайте новую файловую ИБ.
3. При необходимости сразу выполните подготовку:
   - `Restore from DT`;
   - `Update configuration`;
   - `Recreate`;
   - настройку `Launch keys`.
4. Проверьте, что база открывается через `Open in 1C` или `Open in Designer`.
5. После этого та же база будет доступна в picker-ах запуска Vanessa и `Start infobase` у `KOT Form Explorer`.

## 9) KOT Form Explorer (beta): быстрый старт

`KOT Form Explorer` нужен, когда помимо YAML и прогонов нужно посмотреть структуру формы 1С, активный элемент и привязку к `Form.xml`.

Минимальный сценарий:

1. Откройте `KOT - Open 1C Form Explorer` (`KOT - Открыть Исследователь форм 1С`) или кнопку с телескопом в шапке `Test Manager`.
2. Нажмите `Start infobase`.
3. Выберите существующую файловую или серверную базу.
4. Выберите режим установки:
   - `direct`, если база соответствует текущей ветке и нужен самый быстрый старт;
   - `cfe`, если нужен более предсказуемый путь через builder-ИБ и пакет `.cfe`;
   - `target`, если база уже отличается от ветки и runtime нужно собирать по конфигурации самой выбранной базы.
5. KOT установит runtime-расширение и запустит базу:
   - для `direct` и `cfe` источником сборки будет `kotTestToolkit.formExplorer.configurationSourceDirectory`;
   - для `target` KOT сначала выгрузит конфигурацию самой базы, а для файловых ИБ попробует ускорить это через `ibcmd`.
6. После появления snapshot-а можно:
   - переключать `manual` / `auto`;
   - делать `Refresh`;
   - использовать `Locator`;
   - переходить к исходному `Form.xml`.
7. Пока клиент 1С, запущенный из панели, не закрыт, `Start infobase` останется недоступной. Для запуска другой базы сначала закройте текущую сессию.

Если база еще не подготовлена, удобнее сначала сделать это через `KOT Infobase Manager`.

## 10) Повседневная работа

Минимальный цикл:

1. Создаете/Редактируете тестовые сценарии, используя IntelliSense, семантический поиск шагов и KOT Form Explorer.
2. Смотрите диагностику и применяете quick fix.
3. Сохраняете файл (применится авто-форматирование и обновление служебных секций).
4. Собираете тесты `.feature`.
5. Запускаете нужный сценарий в Vanessa.
6. Изучаете результаты прохождения тестов.
7. После массовых изменений (например, Replace in Files) используете `...` -> `Repair scenario files` -> `Repair changed scenarios`.

## 11) Полезные команды контекстного меню

В YAML-редакторе нажмите ПКМ. Базовые быстрые действия:

- `KOT - Open scenario` (`KOT - Открыть сценарий`)
- `KOT - Find references to current scenario` (`KOT - Найти вызовы текущего сценария`)
- `KOT - Add or remove current scenario from favorites` (`KOT - Добавить/убрать открытый сценарий в избранное`)

В `feature`-файлах (ПКМ в редакторе):

- `KOT - Track scenario run by log for opened feature` (`KOT - Отслеживать прогон по логу для открытого feature`)
- `KOT - Switch tracked run for opened feature` (`KOT - Переключить отслеживаемый прогон для открытого feature`)
- `KOT - Stop tracked runs for opened feature` (`KOT - Остановить отслеживание прогонов для открытого feature`)

В `.log`-файлах (ПКМ в редакторе):

- `KOT - Track run by opened log file` (`KOT - Отслеживать прогон по открытому log-файлу`) — использует путь из строки `Feature: ...` и открывает связанный `feature`.

Подменю `KOT - Work with files` (`KOT - Работа с файлами`) - появляется при вызове контекстного меню на выделенном тексте:

- `KOT - Open MXL file in editor` (`KOT - Открыть MXL файл в редакторе`)
- `KOT - Reveal file in VS Code Explorer` (`KOT - Показать файл в проводнике VS Code`)
- `KOT - Reveal file in OS file manager` (`KOT - Показать файл в системном проводнике`)

Для встроенных `.feature`-файлов также доступна команда:

- `KOT - Open nested scenario from feature` (`KOT - Открыть вложенный сценарий из feature`)

Подменю `KOT - Create...` (`KOT - Создать...`):

- `KOT - Create nested scenario` (`KOT - Создать вложенный сценарий`)
- `KOT - Create main scenario` (`KOT - Создать главный сценарий`)

Подменю `KOT - Editing` (`KOT - Редактирование`):

- `KOT - Change main scenario name` (`KOT - Изменить имя главного сценария`) — для главных сценариев
- `KOT - Change nested scenario name` (`KOT - Изменить имя вложенного сценария`) — для вложенных сценариев
- `KOT - Change nested scenario code` (`KOT - Изменить код вложенного сценария`) — для вложенных сценариев
- `KOT - Fill NestedScenarios section` (`KOT - Заполнить секцию ВложенныеСценарии`)
- `KOT - Fill ScenarioParameters section` (`KOT - Заполнить секцию ПараметрыСценария`)

## 12) Частые вопросы

### Почему не видны подсказки/команды?

Проверьте, что файл действительно сценарий (`ТипФайла: "Сценарий"`).

### Почему запуск сценария недоступен?

Сначала выполните `Build tests` (`Собрать тесты`), чтобы появились актуальные артефакты запуска.

### Почему текст в квадратных скобках попадает в параметры?

Если это не параметр, экранируйте скобки: `\[` и `\]`.

### Как искать шаги своими словами?

Используйте префикс `!` перед текстом поиска.  
Пример: `!условие, что открылось окно`, `!проверка фоновых заданий` и плагин предложит шаги из библиотеки шагов Vanessa Automation.

## 13) Что читать дальше

- Полная настройка: [`SETUP.md`](./SETUP.md)
- KOT Form Explorer (beta): [`blocks/form-explorer.md`](./blocks/form-explorer.md)
- Детали по функциональным блокам: [`blocks/README.md`](./blocks/README.md)
- Архитектура и код: [`DEVELOPMENT.md`](./DEVELOPMENT.md)
